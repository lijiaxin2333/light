...
def export_video_without_prefix(
    ...
    # 使用 ffmpeg 重编码 OpenCV 生成的视频，匹配原视频比特率
    if trim_frames > 0:
        reencoded_path = temp_dir_path / "reencoded.mp4"
        reencode_cmd = [
            ffmpeg_path,
            "-y",
            "-i",
        ...
        if original_bitrate:
            # 使用原视频比特率
            reencode_cmd += ["-b:v", str(original_bitrate)]
        else:
            # 无法获取比特率时，使用 CRF 20
            reencode_cmd += ["-crf", "20"]
        reencode_cmd += [
